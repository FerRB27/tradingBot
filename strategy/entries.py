    ...
    def evaluate(self, bar, lr_value, lr_slope, keltner):
        ...
        if self.waiting_pullback == "LONG":
            ...
            if low <= keltner["basis"] <= high:
                # A1: Validar que precio esté en o sobre la LR (EN CONTACTO)
                if price >= lr_value:
        ...
        if self.waiting_pullback == "SHORT":
            # Verificar si la barra toca o cruza la banda media
            if low <= keltner["basis"] <= high:
                # A1: Validar que precio esté en o bajo la LR (EN CONTACTO)
                if price <= lr_value:
                    self.waiting_pullback = None
                    self.impulse_detected = None
                    return "SHORT_A1"
...
class A2Strategy:
    ...
    def evaluate(self, bar, lr_value, lr_slope, keltner):
        """
        Evalúa condiciones A2 según MDC Trading Academy
        ...
        if self.waiting_pullback == "LONG":
            # Verificar si la barra toca o cruza la banda media
            if low <= keltner["basis"] <= high:
                # A2: Validar que precio esté DEBAJO de LR y SIN CONTACTO
        ...
        if self.waiting_pullback == "SHORT":
            # Verificar si la barra toca o cruza la banda media
            if low <= keltner["basis"] <= high:
                # A2: Validar que precio esté ENCIMA de LR y SIN CONTACTO
                # Verificar que ni el close, ni high, ni low toquen la LR
